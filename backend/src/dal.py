...
class ToDoListItems(BaseModel) :
    id:str
    label: str
    checked: bool
    
    @staticmethod
    def from_doc(item) -> "ToDoListItems" :
...
class ToDoList(BaseModel) : 
    ...
    def from_doc(doc) -> "ToDoList" :
        return ToDoList(
            id=str(doc["_id"]),
            name=doc["name"],
            items=[ToDoListItems.from_doc(item) for item in doc["items"]]
        )
...
class ToDoDAL :
    ...
    async def list_todo_list(self, session = None) : 
